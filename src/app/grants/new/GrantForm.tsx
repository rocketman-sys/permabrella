...
import { createGrantAction, type FormState } from "../actions";
    ...
    <form action={formAction} className="mx-auto max-w-xl space-y-4">
      ...
      <div>
        <label htmlFor="locationDetail" className="text-sm font-medium text-[var(--perm-text)]">
          Who can apply / area served (optional)
        </label>
        <Input
          id="locationDetail"
          name="locationDetail"
          placeholder="e.g. Northern Rivers community groups, individuals in Lismore LGA"
          className="mt-1"
        />
      </div>
      <div>
      ...
      <div>
        <label
          ...
          className="text-sm font-medium text-[var(--perm-text)]"
        >
          Application or closing date (optional)
        </label>
        <Input
      ...
      <div>
        <label htmlFor="externalUrl" className="text-sm font-medium text-[var(--perm-text)]">
          Link to apply or learn more (optional)
        </label>
      ...
      <Button type="submit" className="w-full sm:w-auto" disabled={pending}>
        {pending ? "Publishing…" : "Publish grant listing"}
      </Button>
    </form>
  );
}
