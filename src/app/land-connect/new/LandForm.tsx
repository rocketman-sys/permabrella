...
import { createLandListingAction, type FormState } from "../actions";
  ...
  return (
    <form action={formAction} className="mx-auto max-w-xl space-y-4">
      <div>
        ...
        <Select id="listingType" name="listingType" className="mt-1" required defaultValue="land_available">
          <option value="land_available">Land available</option>
          <option value="land_wanted">Land wanted</option>
        </Select>
      </div>
      <div>
      ...
      <Button type="submit" className="w-full sm:w-auto" disabled={pending}>
        {pending ? "Publishing…" : "Publish land listing"}
      </Button>
    </form>
  );
}
